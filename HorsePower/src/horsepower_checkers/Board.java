...
public class Board {
	...
	public double evaluateFor(Boolean player) {
		...
		if (_HPPlayer == player) {
			_temp = (double)(_regsList.size()*2+_kingsList.size()*3)-(_oppRegCount*2+_oppKingCount*3);
		} else {
			_temp = (double)(_oppRegCount*2+_oppKingCount*3)-(_regsList.size()*2+_kingsList.size()*3);
		}
